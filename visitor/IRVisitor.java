...
public class IRVisitor extends GJDepthFirst<String, String>{
    ...
    public IRVisitor(SymbolTable st, String filename){
        ...
        try{
            ...
            if(file.createNewFile()){
                System.out.println("Created new file: " + this.filename);
            }else{
                //If file already exists, delete it and make a new one
                if(file.delete()){
                    file.createNewFile();
                    System.out.println("Overwriting file: " + this.filename);
                }else{
    ...
    //ClassMethod: is a string in "Class::Method" format (c++ like) in order to determine the scope
        ...
        for(String ckey : classkeySet){
            ...
            for(String mkey : methodkeySet){
                ...
                if(st.classes.get(ckey).methods.get(mkey).inherited){
                    ...
                    while(!st.classes.get(parentclass).ParentClass.isEmpty() && st.MethodExists(st.classes.get(parentclass).ParentClass, mkey)){
                        parentclass = st.classes.get(parentclass).ParentClass;
                    }
                    emit(")* @" + parentclass + "." + st.classes.get(ckey).methods.get(mkey).name + " to i8*)");
                }else{
                    emit(")* @" + st.classes.get(ckey).name + "." + st.classes.get(ckey).methods.get(mkey).name + " to i8*)");
                }
    ...
    */
    ...
    public String visit(AssignmentStatement n, String argu) throws Exception{
        ...
        if(st.MethodVariableExists(classname, methodname, lvalue)){
            ...
            //Else: it is a Field of the method. Note: No need to check if the field exists due to secondVisitor
    ...
    */
    ...
    public String visit(ArrayAssignmentStatement n, String argu) throws Exception {
        ...
        if(st.MethodVariableExists(classname, methodname, arr)){
            //Local variable
            int reg = method.genReg();
            emit("\t%" + reg + " = load " + getIRtype(arrType) + ", " + getIRtype(arrType) + "* %" + method.registers.get(arr) + "\n");
            size = reg;
    ...
    */
    public String visit(MessageSend n, String argu) throws Exception {
        ...
        int reg6 = method.genReg();
        emit("\t%" + reg6 + " = call " + getIRtype(msgmethod.type) + " %" + reg5 + "(i8* " + method.callocRegs.get(msgclassname));
    ...
    */
    ...
    public String visit(PrimaryExpression n, String argu) throws Exception {
        ...
        if(n.f0.which == 3){
            ...
            String methodname = getMethod(argu);
            Method method = st.classes.get(classname).methods.get(methodname);
            if(st.MethodVariableExists(classname, methodname, primary) || st.ArgExists(classname, methodname, primary)){
    ...
    */
    ...
    public String visit(AllocationExpression n, String argu) throws Exception {
        ...
        emit("\t%" + reg3 + " = getelementptr [" + numOfmethods + " x i8*], [" + numOfmethods + " x i8*]* @." + new_classname + "_vtable, i32 0, i32 0\n");
